...
For non-interactive macOS/Linux installs that accept all SessionDex installer prompts:
...
For non-interactive macOS/Linux updates:
...
Installer and uninstaller flags are case-sensitive. For example, `./install.sh -Y` is valid, but `./install.sh -y` is not.
