...
fn read_session_history(path: &Path) -> Result<ProviderSessionHistory, String> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        for message in extract_chat_messages(&value) {
            push_history_message(&mut messages, message);
        }
    }
...
fn push_history_message(messages: &mut Vec<ProviderChatMessage>, message: ProviderChatMessage) {
    if messages.last().is_some_and(|last_message| {
        last_message.role == message.role && last_message.text == message.text
    }) {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn session_history_collapses_adjacent_duplicate_messages() {
        let unique_suffix = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system time should be after unix epoch")
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "sessiondex-history-dedupe-test-{}-{unique_suffix}.jsonl",
            std::process::id()
        ));
        let first_line = json!({
            "type": "user_message",
        ...
        let history = read_session_history(&path).expect("test history should be readable");
