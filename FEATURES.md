...
- Search-result session cards open the full chat history when clicked outside their action controls.
...
- The history modal has local message search with previous/next match navigation and match highlighting.
- Each history message bubble can be collapsed and expanded independently.
...
- Metadata filter with Project, Tag, and Collection modes. The value field is searchable and shows up to 10 recent matching suggestions.
- Activity filter:
  - Any time
  - Today
  - Yesterday
...
Session recovery is usually a filtering problem. SessionDex provides direct controls for provider, activity, collection, project, tags, and importance, while keeping folder context searchable.
...
- Case-sensitive non-interactive macOS/Linux install and update mode with `./install.sh -Y` or `./install.sh --yes`.
...
- Case-sensitive non-interactive app removal while keeping data with `--yes` or `-y` on macOS/Linux, and `-Yes` on Windows.
- Case-sensitive optional SessionDex data removal with `--delete-data` on macOS/Linux, and `-DeleteData` on Windows.
