...
$script:Yes = $false
$script:DeleteData = $false

function Show-Usage {
  Write-Host "Usage: .\uninstall.ps1 [-Yes] [-DeleteData]"
  Write-Host ""
  Write-Host "  -Yes         Skip app-removal prompts. This does not delete preferences/data."
  Write-Host "  -DeleteData  Also delete SessionDex preferences, cache, and sessiondex.sqlite3."
}
